    ...
    <Card className="h-full p-5">
      <div className="flex h-full flex-col gap-5">
        <div className="space-y-2">
          ...
          <p className="text-sm text-brand-subtext">Posted {job?.postedTime || 'Recently'}</p>
        </div>

        <p className="line-clamp-2 text-sm text-brand-subtext">
          {job?.fullDescription || job?.description || 'No description provided for this job.'}
        </p>

        <div className="mt-auto flex items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="primary">{job?.budget || 'Budget not specified'}</Badge>
